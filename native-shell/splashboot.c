...
#define SPLASH_MIN_SECONDS 2.0
#define SPLASH_MAX_SECONDS 12.0
...
static gboolean assets_ready(void) {
    return g_file_test(cube_path(), G_FILE_TEST_EXISTS) &&
           g_file_test(wallpaper_path(), G_FILE_TEST_EXISTS) &&
           g_file_test("/usr/local/bin/influent-danenone-firstboot", G_FILE_TEST_IS_EXECUTABLE);
}
...
static gboolean tick(gpointer data) {
    ...
    if ((elapsed >= SPLASH_MIN_SECONDS && assets_ready()) || elapsed >= SPLASH_MAX_SECONDS) {
        state->timer_id = 0;
...
static void activate(GtkApplication *app, gpointer data) {
    ...
    gtk_progress_bar_set_pulse_step(GTK_PROGRESS_BAR(state->progress), 0.12);
    ...
    gtk_css_provider_load_from_data(css,
        ...
        "#splash-root { background: rgba(5,18,22,0.30); border-radius: 0; padding: 26px 40px; }"
        "#splash-veil { background: rgba(4,8,18,0.22); }"
        "#splash-cube { margin-bottom: 10px; }"
        "#splash-progress trough { min-height: 5px; border-radius: 0; background: rgba(255,255,255,0.28); }"
        "#splash-progress progress { min-height: 5px; border-radius: 0; background: #00d084; }",
        -1, NULL);
