...
static void save_visual_preferences(Oobe *o) {
    ...
    gchar *contents = g_strdup_printf("theme=%s\nnotch=%s\nnotch_width=%d\nnotch_height=%d\nnotch_radius=%d\nnotch_border=none\n", theme, notch, notch_width, notch_height, notch_radius);
    if (g_mkdir_with_parents(directory, 0700) == 0) g_file_set_contents(path, contents, -1, NULL);
    g_free(contents); g_free(path); g_free(directory);
}
...
static GtkWidget *choice_row(const char *text) {
...
static GtkWidget *welcome_page(void) {
    return base_page("Configura tu espacio Danenone", "Puedes revisar cada decisión antes de aplicarla. El asistente te acompañará por conectividad, idioma, disco, usuario, privacidad y apariencia.");
}

static GtkWidget *network_page(void) {
    return base_page("Conecta el sistema", "La conectividad se comprobará usando NetworkManager para verificar paquetes, idiomas y herramientas adicionales.");
}

static GtkWidget *language_page(void) {
    GtkWidget *box = base_page("Elige el idioma y la región", "Selecciona la interfaz que quieres usar. English permanece disponible sin conexión; los demás paquetes se comprueban antes de descargarse.");
    GtkWidget *list = gtk_list_box_new(); gtk_widget_add_css_class(list, "choice-list");
    const char *langs[] = {"Español — Latinoamérica", "Español — España", "English — United States", "Português — Brasil"};
    for (guint i = 0; i < G_N_ELEMENTS(langs); i++) gtk_list_box_append(GTK_LIST_BOX(list), choice_row(langs[i]));
    gtk_box_append(GTK_BOX(box), list); return box;
}
...
static GtkWidget *license_page(void) {
    ...
    GtkWidget *license = gtk_check_button_new_with_label("Acepto los términos de licencia y la política de privacidad.");
    gtk_widget_add_css_class(license, "setting-row");
    gtk_box_append(GTK_BOX(box), license); return box;
}
...
static GtkWidget *disk_page(void) {
    ...
    GtkWidget *disk = gtk_check_button_new_with_label("NVMe SSD  /dev/nvme0n1  ·  512 GB"); gtk_check_button_set_active(GTK_CHECK_BUTTON(disk), TRUE);
...
static GtkWidget *user_page(void) {
    ...
    GtkWidget *name = gtk_entry_new(); gtk_entry_set_placeholder_text(GTK_ENTRY(name), "Nombre de usuario"); gtk_widget_add_css_class(name, "input");
    GtkWidget *pass = gtk_entry_new(); gtk_entry_set_visibility(GTK_ENTRY(pass), FALSE); gtk_entry_set_placeholder_text(GTK_ENTRY(pass), "Contraseña local"); gtk_widget_add_css_class(pass, "input");
    gtk_box_append(GTK_BOX(box), name); gtk_box_append(GTK_BOX(box), pass); return box;
}
...
static GtkWidget *summary_page(void) {
    ...
    GtkWidget *summary = text_label("Sistema: Influent Danenone\nBase: Arch Linux\nDestino: /dev/nvme0n1\nModo: instalación completa o dual boot según tu elección\nFormato previsto: Btrfs con compresión ZSTD", "summary-card");
    gtk_box_append(GTK_BOX(box), summary); return box;
}
...
static GtkWidget *install_page(void) {
    ...
    gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(progress), 0.45); gtk_widget_set_size_request(progress, -1, 8);
    gtk_box_append(GTK_BOX(box), progress); return box;
}

static void set_page(Oobe *o, int page_no) {
    o->page = CLAMP(page_no, 0, PAGE_COUNT - 1);
    const char *names[] = {"welcome", "network", "language", "license", "disk", "user", "tools", "appearance", "notch", "privacy", "summary", "install"};
    const char *stages[] = {"Paso 1 de 12 · Bienvenida", "Paso 2 de 12 · Conectividad", "Paso 3 de 12 · Idioma", "Paso 4 de 12 · Licencia", "Paso 5 de 12 · Disco", "Paso 6 de 12 · Usuario", "Paso 7 de 12 · Herramientas", "Paso 8 de 12 · Apariencia", "Paso 9 de 12 · Notch", "Paso 10 de 12 · Privacidad", "Paso 11 de 12 · Resumen", "Paso 12 de 12 · Instalación"};
    ...
    const char *status[] = {"Puedes revisar cada decisión antes de aplicarla. Todo queda bajo tu control.", "La conexión se comprobará usando NetworkManager.", "Los paquetes de idioma se verifican antes de instalarse.", "Debes aceptar los términos antes de continuar.", "La instalación real confirmará el disco y el modo elegido.", "El usuario local se crea sin una cuenta remota.", "Las herramientas opcionales se instalarán después del sistema base.", "El modo claro/oscuro se aplica automáticamente como prueba.", "El contenido se reajusta al editar el estilo del notch.", "Tus preferencias permanecen locales por defecto.", "Revisa el destino, el usuario, la privacidad y el aspecto.", "Aplicando el entorno, la barra, el notch y el tour."};
...
static void next_clicked(GtkButton *button, gpointer data) {
    ...
    if (o->page < PAGE_COUNT - 1) set_page(o, o->page + 1);
    else { gtk_widget_set_visible(o->content, FALSE); gtk_widget_set_visible(o->notch, FALSE); gtk_widget_set_visible(o->final_splash, TRUE); o->final_timer = g_timeout_add(40, final_step, o->final_progress); }
}
...
static void dark_toggled(GtkCheckButton *button, gpointer data) {
    Oobe *o = data; if (gtk_check_button_get_active(button)) gtk_widget_add_css_class(o->root, "dark"); else gtk_widget_remove_css_class(o->root, "dark"); save_visual_preferences(o);
}
...
static void full_notch_toggled(GtkCheckButton *button, gpointer data) {
    ...
    gtk_widget_set_size_request(o->notch, 360, 28); gtk_widget_remove_css_class(o->notch, "dynamic-notch");
    gtk_widget_set_margin_top(o->content, 30); save_visual_preferences(o);
}

static void dynamic_notch_toggled(GtkCheckButton *button, gpointer data) {
    Oobe *o = data; if (!gtk_check_button_get_active(button)) return;
    gtk_widget_set_size_request(o->notch, 220, 28); gtk_widget_add_css_class(o->notch, "dynamic-notch");
    gtk_widget_set_margin_top(o->content, 18); save_visual_preferences(o);
}
...
static void load_css(void) {
    const char *css =
        ...
        ".splash-progress trough { min-width: 220px; min-height: 3px; border: 0; border-radius: 0; background: rgba(255,255,255,0.38); }"
        ".splash-progress progress { min-height: 3px; border: 0; border-radius: 0; background: #00b982; }"
        ...
        ".identity { min-height: 64px; } .brand { color: #17342d; font-family: Roboto, 'Noto Sans', sans-serif; font-size: 26px; font-weight: 500; }"
        ".stage { color: rgba(23,52,45,0.62); font-size: 13px; } .page { background: rgba(255,255,255,0.22); border: 1px solid rgba(255,255,255,0.58); padding: 16px; }"
        ".page-title { color: #17342d; font-size: 18px; font-weight: 700; } .body-text { color: rgba(20,45,39,0.78); font-size: 15px; } .status { color: rgba(20,45,39,0.68); font-size: 14px; }"
        ".choice-list, .choice-card, .summary-card { background: rgba(255,255,255,0.34); border: 1px solid rgba(255,255,255,0.64); } .choice-card { padding: 12px; } .summary-card { padding: 16px; color: #17342d; }"
        ".input { padding: 10px; background: rgba(255,255,255,0.40); border: 1px solid rgba(255,255,255,0.70); }"
        "button { padding: 10px 17px; border-radius: 3px; border: 1px solid rgba(255,255,255,0.82); box-shadow: 0 4px 14px rgba(31,75,66,0.12), inset 0 1px 0 rgba(255,255,255,0.88); }"
        ".primary { background: rgba(0,184,132,0.84); color: #07352a; border-bottom: 2px solid #008a5a; font-weight: 700; } .secondary { background: rgba(255,255,255,0.34); color: #17342d; border-bottom: 2px solid rgba(0,112,86,0.42); }"
        ".install-progress trough { min-height: 6px; border-radius: 0; background: rgba(20,65,55,0.16); } .install-progress progress { min-height: 6px; border-radius: 0; background: #00b982; }"
        ".final-title { color: #17342d; font-size: 20px; font-weight: 700; }"
        ".dark .oobe-modal { background: rgba(20,29,28,0.88); border-color: rgba(255,255,255,0.22); } .dark .page, .dark .choice-list, .dark .choice-card, .dark .summary-card { background: rgba(13,23,22,0.42); border-color: rgba(255,255,255,0.18); } .dark .page-title, .dark .body-text, .dark .status, .dark .brand, .dark .summary-card { color: #ecf8f3; }";
...
static void activate(GtkApplication *app, gpointer data) {
    (void)data; Oobe *o = g_new0(Oobe, 1); o->app = app;
    ...
    o->overlay = GTK_WIDGET(gtk_overlay_new());
    GtkWidget *wallpaper = gtk_picture_new_for_filename(asset(OOBE_WALLPAPER_PATH, LOCAL_WALLPAPER_PATH)); gtk_picture_set_content_fit(GTK_PICTURE(wallpaper), GTK_CONTENT_FIT_COVER); gtk_widget_set_hexpand(wallpaper, TRUE); gtk_widget_set_vexpand(wallpaper, TRUE); gtk_overlay_set_child(GTK_OVERLAY(o->overlay), wallpaper);
    ...
    o->splash = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0); gtk_widget_add_css_class(o->splash, "oobe-splash"); gtk_widget_set_hexpand(o->splash, TRUE); gtk_widget_set_vexpand(o->splash, TRUE);
    GtkWidget *sp = gtk_box_new(GTK_ORIENTATION_VERTICAL, 22); gtk_widget_add_css_class(sp, "splash-panel"); gtk_widget_set_halign(sp, GTK_ALIGN_CENTER); gtk_widget_set_valign(sp, GTK_ALIGN_CENTER); gtk_box_append(GTK_BOX(sp), scaled_picture(asset(BRAND_LOGO_PATH, LOCAL_LOGO_PATH), 128));
    o->splash_progress = gtk_progress_bar_new(); gtk_widget_add_css_class(o->splash_progress, "splash-progress"); gtk_widget_set_size_request(o->splash_progress, 220, 6); gtk_widget_set_margin_top(o->splash_progress, 8); gtk_progress_bar_set_pulse_step(GTK_PROGRESS_BAR(o->splash_progress), 0.10); gtk_box_append(GTK_BOX(sp), o->splash_progress); gtk_box_append(GTK_BOX(o->splash), sp); gtk_overlay_add_overlay(GTK_OVERLAY(o->overlay), o->splash);
    ...
    gtk_stack_add_named(GTK_STACK(o->stack), welcome_page(), "welcome"); gtk_stack_add_named(GTK_STACK(o->stack), network_page(), "network"); gtk_stack_add_named(GTK_STACK(o->stack), language_page(), "language"); gtk_stack_add_named(GTK_STACK(o->stack), license_page(), "license"); gtk_stack_add_named(GTK_STACK(o->stack), disk_page(), "disk"); gtk_stack_add_named(GTK_STACK(o->stack), user_page(), "user"); gtk_stack_add_named(GTK_STACK(o->stack), tools_page(), "tools"); gtk_stack_add_named(GTK_STACK(o->stack), appearance_page(o), "appearance"); gtk_stack_add_named(GTK_STACK(o->stack), notch_page(o), "notch"); gtk_stack_add_named(GTK_STACK(o->stack), privacy_page(), "privacy"); gtk_stack_add_named(GTK_STACK(o->stack), summary_page(), "summary"); gtk_stack_add_named(GTK_STACK(o->stack), install_page(), "install"); gtk_box_append(GTK_BOX(o->modal), o->stack);
    ...
    gtk_window_set_child(GTK_WINDOW(o->window), o->overlay); gtk_window_present(GTK_WINDOW(o->window)); o->splash_timer = g_timeout_add(4500, finish_splash, o); o->marquee_timer = g_timeout_add(34, marquee_step, o);
    const char *start = g_getenv("DANENONE_OOBE_PAGE"); int initial = start ? CLAMP((int)g_ascii_strtoll(start, NULL, 10), 0, PAGE_COUNT - 1) : 0; set_page(o, initial);
}
