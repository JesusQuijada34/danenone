...
static const char *const tr_en[][2] = {
    ...
    {"storage_title", "Installation storage"}, {"storage_body", "Select the device and a safe installation plan. Formatting is not executed by the OOBE; the installer must confirm the final action."}, {"storage_tool_device", "Devices"}, {"storage_tool_partition", "Partitions"}, {"storage_tool_resize", "Resize"}, {"storage_tool_format", "Format"},
...
static gchar *list_disks(void) {
    ...
    gint status = 1;
    GError *error = NULL;
    g_spawn_command_line_sync("lsblk -dn -o NAME,SIZE,TYPE", &out, NULL, &status, &error);
    if (error) g_error_free(error);
    GString *result = g_string_new("");
    if (status == 0 && out) {
        gchar **lines = g_strsplit(out, "\n", -1);
        for (guint i = 0; lines[i]; i++) {
            char name[128] = {0}, size[128] = {0}, type[128] = {0};
            if (sscanf(lines[i], "%127s %127s %127s", name, size, type) == 3 && g_strcmp0(type, "disk") == 0) {
                if (result->len) g_string_append_c(result, '\n');
                g_string_append_printf(result, "/dev/%s · %s", name, size);
            }
        }
        g_strfreev(lines);
    }
...
static GtkWidget *storage_tile(Oobe *o, const char *label_key, const char *icon_installed, const char *icon_local, GtkWidget *control) { GtkWidget *tile = gtk_box_new(GTK_ORIENTATION_VERTICAL, 6); gtk_widget_add_css_class(tile, "storage-tile"); gtk_widget_set_size_request(tile, 190, 76); GtkWidget *head = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 8); GtkWidget *icon = scaled_picture(asset(icon_installed, icon_local), 28); gtk_widget_add_css_class(icon, "storage-icon"); gtk_box_append(GTK_BOX(head), icon); gtk_box_append(GTK_BOX(head), label_for(o, label_key, "storage-tile-label")); gtk_box_append(GTK_BOX(tile), head); gtk_widget_set_hexpand(control, TRUE); gtk_box_append(GTK_BOX(tile), control); return tile; }
...
static GtkWidget *storage_page(Oobe *o) { GtkWidget *box = base_page(o, "storage_title", "storage_body"); gtk_widget_add_css_class(box, "storage-page"); gchar *disk_text = list_disks(); gchar **disks = g_strsplit(disk_text, "\n", -1); o->disk_dropdown = make_dropdown((const char *const *)disks); g_strfreev(disks); g_free(disk_text); gchar *partition_text = list_disks(); gchar **partitions = g_strsplit(partition_text, "\n", -1); o->partition_target = make_dropdown((const char *const *)partitions); g_strfreev(partitions); g_free(partition_text); o->partition_plan = make_translated_dropdown(o, PARTITION_KEYS, &o->partition_model); o->resize_size = gtk_spin_button_new_with_range(16, 4096, 1); gtk_spin_button_set_value(GTK_SPIN_BUTTON(o->resize_size), 64); gtk_widget_set_tooltip_text(o->resize_size, "Tamaño de redimensionado en GiB"); o->format_plan = make_translated_dropdown(o, FORMAT_KEYS, &o->format_model); o->format_confirm = gtk_check_button_new_with_label(tr(o, "format_confirm")); bind_widget(o, o->format_confirm, "format_confirm"); GtkWidget *grid = gtk_flow_box_new(); gtk_widget_add_css_class(grid, "storage-grid"); gtk_flow_box_set_selection_mode(GTK_FLOW_BOX(grid), GTK_SELECTION_NONE); gtk_flow_box_set_row_spacing(GTK_FLOW_BOX(grid), 6); gtk_flow_box_set_column_spacing(GTK_FLOW_BOX(grid), 6); gtk_flow_box_set_max_children_per_line(GTK_FLOW_BOX(grid), 3); gtk_flow_box_set_min_children_per_line(GTK_FLOW_BOX(grid), 3); gtk_flow_box_insert(GTK_FLOW_BOX(grid), storage_tile(o, "device_label", STORAGE_FILES_ICON, LOCAL_STORAGE_FILES_ICON, o->disk_dropdown), -1); gtk_flow_box_insert(GTK_FLOW_BOX(grid), storage_tile(o, "partition_label", STORAGE_FILES_ICON, LOCAL_STORAGE_FILES_ICON, o->partition_target), -1); gtk_flow_box_insert(GTK_FLOW_BOX(grid), storage_tile(o, "storage_auto", STORAGE_CONTROL_ICON, LOCAL_STORAGE_CONTROL_ICON, o->partition_plan), -1); gtk_flow_box_insert(GTK_FLOW_BOX(grid), storage_tile(o, "storage_resize", STORAGE_SETTINGS_ICON, LOCAL_STORAGE_SETTINGS_ICON, o->resize_size), -1); gtk_flow_box_insert(GTK_FLOW_BOX(grid), storage_tile(o, "format_label", STORAGE_CONTROL_ICON, LOCAL_STORAGE_CONTROL_ICON, o->format_plan), -1); gtk_box_append(GTK_BOX(box), grid); gtk_box_append(GTK_BOX(box), o->format_confirm); GtkWidget *tools = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 8); gtk_widget_add_css_class(tools, "storage-tools"); gtk_widget_set_halign(tools, GTK_ALIGN_CENTER); gtk_widget_set_margin_top(tools, 2); gtk_widget_set_margin_bottom(tools, 2); gtk_box_append(GTK_BOX(tools), storage_tool_button(o, "storage_tool_device", STORAGE_FILES_ICON, LOCAL_STORAGE_FILES_ICON, o->disk_dropdown)); gtk_box_append(GTK_BOX(tools), storage_tool_button(o, "storage_tool_partition", STORAGE_FILES_ICON, LOCAL_STORAGE_FILES_ICON, o->partition_target)); gtk_box_append(GTK_BOX(tools), storage_tool_button(o, "storage_tool_resize", STORAGE_SETTINGS_ICON, LOCAL_STORAGE_SETTINGS_ICON, o->resize_size)); gtk_box_append(GTK_BOX(tools), storage_tool_button(o, "storage_tool_format", STORAGE_CONTROL_ICON, LOCAL_STORAGE_CONTROL_ICON, o->format_plan)); gtk_box_append(GTK_BOX(box), tools); o->disk_status = label_for(o, "storage_safe", "status-card"); gtk_box_append(GTK_BOX(box), o->disk_status); return box; }
...
static void load_css(Oobe *o) {
    ...
    const char *css =
        ...
        ".oobe-page { background: rgba(255,255,255,.22); border: 1px solid rgba(255,255,255,.58); padding: 12px; } .oobe-footer { min-height: 42px; } scrolledwindow undershoot { background: transparent; } .storage-page { padding-top: 8px; padding-bottom: 8px; } .storage-page .body-text { font-size: 14px; } .storage-page .dropdown { padding: 5px 8px; } .storage-grid { background: rgba(255,255,255,.16); border: 1px solid rgba(255,255,255,.58); padding: 6px; } .storage-tile { background: rgba(255,255,255,.42); border: 1px solid rgba(255,255,255,.72); padding: 6px; min-height: 76px; } .storage-tile-label { color: #17342d; font-size: 12px; font-weight: 700; } .storage-icon { min-width: 24px; min-height: 24px; } .storage-tools { background: rgba(255,255,255,.24); border: 1px solid rgba(255,255,255,.54); padding: 5px; } .storage-tool { min-width: 96px; padding: 3px 7px; background: rgba(255,255,255,.42); } .storage-tool-label { color: #17342d; font-size: 11px; font-weight: 600; }"
        ...
        ".dark .oobe-page, .dark .summary-card, .dark .status-card, .dark .network-list { background: rgba(13,23,22,.42); border-color: rgba(255,255,255,.18); } .dark .storage-grid, .dark .storage-tools { background: rgba(13,23,22,.32); border-color: rgba(255,255,255,.18); } .dark .storage-tile, .dark .storage-tool { background: rgba(13,23,22,.52); border-color: rgba(255,255,255,.20); } .dark .storage-tile-label, .dark .storage-tool-label { color: #ecf8f3; }"
