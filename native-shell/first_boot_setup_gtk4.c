...
static const char *const tr_en[][2] = {
    ...
    {"storage_title", "Installation storage"}, {"storage_body", "Select the device and a safe installation plan. Formatting is not executed by the OOBE; the installer must confirm the final action."},
...
static const char *const tr_es[][2] = {
 ...
 {"edition_body", "La edición define los paquetes y la política de actualizaciones de la instalación."}, {"edition_label", "Edición de destino"},
    {"storage_title", "Almacenamiento de instalación"}, {"storage_body", "Selecciona el dispositivo y un plan seguro. El OOBE no ejecuta formateos; el instalador debe confirmar la acción final."}, {"device_label", "Dispositivo de destino"}, {"partition_label", "Plan de partición"}, {"format_label", "Sistema de archivos y formato"}, {"storage_safe", "No se ha ejecutado ninguna operación de disco. Se requiere la confirmación final del instalador."},
...
static GtkWidget *storage_page(Oobe *o) { GtkWidget *box = base_page(o, "storage_title", "storage_body"); gtk_box_append(GTK_BOX(box), label_for(o, "device_label", "field-label")); gchar *disk_text = list_disks(); gchar **disks = g_strsplit(disk_text, "\n", -1); o->disk_dropdown = make_dropdown((const char *const *)disks); g_strfreev(disks); g_free(disk_text); gtk_box_append(GTK_BOX(box), o->disk_dropdown); gtk_box_append(GTK_BOX(box), label_for(o, "partition_label", "field-label")); gchar *partition_text = list_disks(); gchar **partitions = g_strsplit(partition_text, "\n", -1); o->partition_target = make_dropdown((const char *const *)partitions); g_strfreev(partitions); g_free(partition_text); gtk_box_append(GTK_BOX(box), o->partition_target); o->partition_plan = make_translated_dropdown(o, PARTITION_KEYS, &o->partition_model); gtk_box_append(GTK_BOX(box), o->partition_plan); o->resize_size = gtk_spin_button_new_with_range(16, 4096, 1); gtk_spin_button_set_value(GTK_SPIN_BUTTON(o->resize_size), 64); gtk_widget_set_tooltip_text(o->resize_size, "Tamaño de redimensionado en GiB"); gtk_box_append(GTK_BOX(box), o->resize_size); gtk_box_append(GTK_BOX(box), label_for(o, "format_label", "field-label")); o->format_plan = make_translated_dropdown(o, FORMAT_KEYS, &o->format_model); gtk_box_append(GTK_BOX(box), o->format_plan); o->format_confirm = gtk_check_button_new_with_label(tr(o, "format_confirm")); bind_widget(o, o->format_confirm, "format_confirm"); gtk_box_append(GTK_BOX(box), o->format_confirm); o->disk_status = label_for(o, "storage_safe", "status-card"); gtk_box_append(GTK_BOX(box), o->disk_status); return box; }
...
static void load_css(Oobe *o) {
    ...
    const char *css =
        ...
        ".oobe-modal { background: rgba(255,255,255,.80); border: 1px solid rgba(255,255,255,.92); border-radius: 4px; padding: 26px; box-shadow: 0 18px 50px rgba(13,45,42,.18); }"
        ...
        ".oobe-page { background: rgba(255,255,255,.22); border: 1px solid rgba(255,255,255,.58); padding: 16px; }"
        ...
        ".dark .oobe-page, .dark .summary-card, .dark .status-card, .dark .network-list { background: rgba(13,23,22,.42); border-color: rgba(255,255,255,.18); }"
